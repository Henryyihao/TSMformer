...
class LinformerAttention(nn.Module):
    def __init__(self, proj_len, dropout=0.1, k=64):
        """
        参数:
          proj_len: 输入序列长度（经过 encoder_embedding 后的序列长度，一般为 nums_feature）
          dropout: dropout 概率
          k: 投影后的低维度
        """
        ...
        # 用于 keys 和 values 的投影矩阵，形状均为 (k, proj_len)
        self.E_k = nn.Parameter(torch.randn(self.k, self.proj_len))
        self.E_v = nn.Parameter(torch.randn(self.k, self.proj_len))

    def forward(self, queries, keys, values, attn_mask=None, tau=None, delta=None):
        """
        参数:
          queries: (B, L, H, d_k)
          keys:    (B, S, H, d_k)  其中 S 应等于 self.proj_len
          values:  (B, S, H, d_v)
          attn_mask: 可选的注意力 mask（需与投影后的 scores 对应）
        返回:
          out: 输出张量，形状 (B, L, H, d_v)
          attn: 注意力权重，形状 (B, L, H, k)
        """
        B, L, H, d_k = queries.shape
        S = keys.shape[1]
        assert S == self.proj_len, "输入序列长度必须与初始化时的 proj_len 保持一致"

        # 将 keys 和 values 变换为 (B, H, S, d_k) / (B, H, S, d_v)
        keys = keys.permute(0, 2, 1, 3)
        values = values.permute(0, 2, 1, 3)

        # 对 keys 和 values 进行线性投影: 从 (B, H, S, d_k)/(B, H, S, d_v) 到 (B, H, k, d_k)/(B, H, k, d_v)
        projected_keys = torch.einsum("ks,bhsd->bhkd", self.E_k, keys)
        projected_values = torch.einsum("ks,bhsd->bhkd", self.E_v, values)

        # 计算缩放点积注意力分数，形状为 (B, L, H, k)
        scores = torch.einsum("blhd,bhkd->blhk", queries, projected_keys) / math.sqrt(d_k)

        # 如果提供了注意力 mask，则应用 mask
        ...
        # 得到输出: (B, L, H, d_v)
        out = torch.einsum("blhk,bhkd->blhd", attn, projected_values)
