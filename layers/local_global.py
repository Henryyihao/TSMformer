...
class moving_avg(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_len = self.kernel_size - 1  # Total padding required
        left_pad = pad_len // 2  # Half of the padding on the left
        right_pad = pad_len - left_pad  # Remaining padding on the right

        # Apply padding to keep output the same length as input
        x = F.pad(x, (0, 0, left_pad, right_pad), mode='replicate')

        # AvgPool1d operates on [batch, channels, seq_len]
        x = self.avg(x.permute(0, 2, 1))  # Permute to [batch, channels, seq_len]
        x = x.permute(0, 2, 1)  # Permute back to original shape

        # Ensure the output has the same sequence length as the input
        return x[:, :seq_len, :]  # Truncate any extra length added by padding
...
class MIC(nn.Module):
    ...
    def conv_trans_conv(self, input, conv1d, conv1d_trans, isometric):
        ...
        # isometric convolution
        padding_size = (x.shape[2] - 1) * isometric.stride[0] + isometric.kernel_size[0] - 2 * isometric.padding[0]
        zeros = torch.zeros((x.shape[0], x.shape[1], int(padding_size - x.shape[2])), device=self.device)
